...
- The required game folder location is `<web-workspace-root>/<game-slug>/`, where `<web-workspace-root>` comes from the reporter. If that folder does not exist, create it in the reported web workspace.
- The final zip must also live inside that same reported web workspace game folder as `<web-workspace-root>/<game-slug>/<game-slug>.zip`.
...
- Write the generated game into the reporter-confirmed web page workspace as real files under `<web-workspace-root>/<game-slug>/`.
